...
import { SelectionControl } from "./CardViews";
...
export function AdvancedFilters({ cards, filters, setFilters, onClose, onReset }: AdvancedFiltersProps) {
  const homebrew = cards.length > 0 && cards.every((card) => card.homebrew);
  ...
  return (
    <ModalFrame className="filter-sheet" labelledBy="filter-heading" onClose={onClose}>
      <header className="sheet-header"><div><p className="eyebrow">Advanced search</p><h2 id="filter-heading">Find the exact {homebrew ? "card or variant" : "print"}</h2><p>{homebrew ? "Search rules text, set, variant policy, designer, and color identity. Results update as you refine." : "Search rules text, print details, condition, price, and color identity. Results update as you refine."}</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close advanced search">×</button></header>
...
export function CardPreview({ card, selected, wanted, onClose, onToggleSelected, onToggleWanted }: CardPreviewProps) {
  const unavailable = card.tradability.key === "not_available";
  const [faceIndex, setFaceIndex] = useState(card.activeFaceIndex || card.faces?.[0]?.faceIndex || 0);
  const faces = card.faces || [];
  const facePosition = Math.max(0, faces.findIndex((face) => face.faceIndex === faceIndex));
  const face = faces[facePosition];
  const imageUrl = face?.largeImageUrl || card.largeImageUrl || card.imageUrl;
  const showNextFace = () => setFaceIndex(faces[(facePosition + 1) % faces.length]?.faceIndex || 0);
  return <ModalFrame className="preview-dialog" labelledBy="preview-card-title" onClose={onClose}><button type="button" className="icon-button preview-close" onClick={onClose} aria-label="Close card preview">×</button><div className="preview-image-stage"><img src={imageUrl} alt={`${card.name}${facePosition ? " back face" : " card"}`} />{faces.length > 1 && <button type="button" className="preview-face-toggle" onClick={showNextFace}><span aria-hidden="true">↻</span> Show {facePosition === 0 ? "back" : "front"} <small>{facePosition + 1} of {faces.length}</small></button>}</div><div className="preview-copy"><p className="eyebrow">{card.setName} · {card.setCode.toUpperCase()} {card.collectorNumber}</p><h2 id="preview-card-title">{face?.faceName || card.name}</h2><p>{face?.typeLine || card.typeLine}</p>{card.homebrew && <p className="variant-copy"><b>{card.variantName}</b> · {titleCase(card.variantPolicy || "default")} export · {card.variantCount || 1} {(card.variantCount || 1) === 1 ? "variant" : "variants"}</p>}<p className={`inquiry-copy ${unavailable ? "not-tradable-copy" : ""}`}>{card.tradability.label} · {card.tradability.reason}</p><dl><div><dt>{card.homebrew ? "Designer" : "Owner"}</dt><dd>{card.designer || card.owner}</dd></div><div><dt>{card.homebrew ? "Faces" : "Quantity"}</dt><dd>{card.homebrew ? faces.length || 1 : `${card.quantity} ${card.quantity === 1 ? "copy" : "copies"}`}</dd></div><div><dt>{card.homebrew ? "Variant status" : "Finish"}</dt><dd>{titleCase(card.homebrew ? card.variantStatus || "active" : card.finish)}</dd></div><div><dt>{card.homebrew ? "Source" : "Condition"}</dt><dd>{card.homebrew ? "Homebrew Forge" : titleCase(card.condition)}</dd></div>{!card.homebrew && <><div><dt>From</dt><dd>{sourceLabel(card.sourceBinders)}</dd></div><div><dt>Market snapshot</dt><dd>{formatMoney(card.marketPrice)}</dd></div></>}</dl><div className="dialog-button-row"><button type="button" className="secondary-action" onClick={onToggleWanted}>{wanted ? "Remove from wants" : "Add to wants"}</button><SelectionControl className="dialog-selection" cardName={card.name} selected={selected} onToggle={onToggleSelected} label={selected ? "Selected" : "Select card"} /></div></div></ModalFrame>;
}
...
export function TradeList({ cards, email, githubUrl, onClose, onClear, onSaveWants }: TradeListProps) {
  ...
  return <ModalFrame className="trade-dialog" labelledBy="trade-list-heading" onClose={onClose}><button type="button" className="icon-button preview-close" onClick={onClose} aria-label="Close selected cards">×</button><p className="eyebrow">Selected cards</p><h2 id="trade-list-heading">{cards.length} {cards.length === 1 ? "card" : "cards"} ready to use</h2><p className="dialog-copy">Download or share this exact selection, save it as a wants list, or use it to start a conversation. Official-card selection never confirms availability, and homebrew selection never confirms a physical print exists.</p><div className="request-list">{cards.map((card) => <div key={card.id}><span>{card.name}</span><small>{card.setCode.toUpperCase()} {card.collectorNumber} · {card.homebrew ? `${card.variantName} · ${titleCase(card.variantPolicy || "default")} export` : `${titleCase(card.finish)} · ${formatMoney(card.marketPrice)}`}</small></div>)}</div><div className="trade-dialog-actions"><button type="button" className="primary-action" onClick={copy}>Copy selection summary</button>{email ? <a className="secondary-action" href={mailHref}>Open email draft</a> : <a className="secondary-action" href={githubUrl} target="_blank" rel="noreferrer">Open contact profile</a>}<button type="button" className="secondary-action" onClick={() => { onSaveWants(); setNotice("Saved to this device’s wants list."); }}>Add to wants list</button><button type="button" className="text-action" onClick={onClear}>Clear selection</button></div><div className="export-row" aria-label="Download selected cards"><span>Download selection:</span><button type="button" className="text-action" onClick={() => exportSelection("csv")}>CSV</button><button type="button" className="text-action" onClick={() => exportSelection("text")}>Text</button><button type="button" className="text-action" onClick={() => exportSelection("xml")}>XML</button></div><p className="dialog-status" role="status">{notice}</p></ModalFrame>;
}
