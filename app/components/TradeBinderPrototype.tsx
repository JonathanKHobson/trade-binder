...
import { consolidatePrints, isTradeRequestable } from "../data/inventory";
...
export function TradeBinderPrototype() {
  ...
  const toggleSelected = (id: string) => {
    ...
    if (!card || !isTradeRequestable(card)) return;
  ...
  return <main className="app-shell" data-theme={theme}>
    ...
    <div className="binder-layout">
      <aside className="binder-sidebar"><NavButtons activeSection={activeSection} onNavigate={navigate} /><section className="owned-summary"><span>Cards owned</span><strong>{data.summary.totalQuantity.toLocaleString()}</strong><p>Owner and trade status are recorded for every print. Only requestable cards can enter a trade list.</p><hr /><small>Collection snapshot<br />July 25, 2026</small></section></aside>
      <section className="workspace" id="main-content" aria-label="Trade Binder workspace">
        ...
        {activeSection === "browse" && <>
          <div className="inquiry-banner"><b>Trade status is explicit.</b><span>Cards marked Not tradable stay visible for reference but cannot be added to a trade request.</span></div>
